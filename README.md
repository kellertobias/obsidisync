...
3. On that page, sign in: password servers show the normal login form, OIDC servers send you to the identity provider and back (see the note on the redirect URI below), and static-token servers ask for the token. The page then uses your default vault (the one you synced most recently; a choice is offered only if you have several), and asks for a device name, the **sync folder** for Saber's encrypted files (default `Tablet/.sync`, hidden in Obsidian's file explorer), the **PDF folder** for rendered notes (default `Tablet`), and the **Saber encryption password** you are going to use. Leave the encryption password empty to only store the encrypted files without PDFs.
...
- In OIDC mode the browser pages (`/login`, `/change-feed`, the Saber login page) sign in through the authorization code flow with PKCE using the plugin's `OIDC_DEVICE_CLIENT_ID` and `OIDC_DEVICE_SCOPE`. Register `https://<server>/login/oidc/callback` as a redirect URI of that client in the identity provider (in Zitadel: the application's **Redirect URIs**), otherwise the provider refuses the login.
